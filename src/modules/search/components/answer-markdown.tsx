...
export const AnswerMarkdown = React.memo(function AnswerMarkdown({ text, className, streaming }: { text: string; className?: string; streaming?: boolean }) {
  ...
  return (
    <div className={cn("prose-legal font-serif text-[15px] leading-[1.7] text-foreground [&>*:first-child]:mt-0 [&>*:last-child]:mb-0", streaming && "answer-streaming", className)}>
      <ReactMarkdown remarkPlugins={REMARK} components={COMPONENTS}>{prepared}</ReactMarkdown>
    </div>
  );
});
...
function Sentences({ children }: { children: React.ReactNode }) {
  const a = useResearchActions();
  // Always render the grouped structure so hovering never remounts the citation chips (a remount would swallow the click).
  const groups = React.useMemo(() => groupSentences(React.Children.toArray(children)), [children]);
  if (!groups.some((g) => g.cites.size)) return <>{children}</>;
  return (
    <>
      {groups.map((g, i) => (
        <span key={i} className={cn("rounded-sm transition-colors", a.hoverN != null && g.cites.has(a.hoverN) && "bg-primary/10 box-decoration-clone px-0.5 -mx-0.5")}>{g.nodes}</span>
      ))}
    </>
  );
}
