  ...
  return (
    <div className="rounded-md border">
      ...
      <div role="table" aria-label="Providers" className="text-[12.5px]">
        <div role="row" className="grid h-7 grid-cols-[150px_minmax(0,1fr)_170px_150px] items-center gap-3 border-b px-3 grid-head">
          <span>Provider</span><span>Used for</span><span>Variable</span><span>Status</span>
        </div>
        {rows.map((p) => {
          const st = STATE_LABEL[p.state];
          return (
            <div key={p.id} role="row" className="grid min-h-7 grid-cols-[150px_minmax(0,1fr)_170px_150px] items-center gap-3 border-b border-line-quiet px-3 py-0.5 last:border-b-0">
              <span className="truncate font-medium">{p.name}</span>
              <span className="truncate text-muted-foreground" title={p.role}>{p.role}</span>
              <span className="truncate font-mono text-[11px] text-muted-foreground">{p.env.join(", ") || "—"}</span>
