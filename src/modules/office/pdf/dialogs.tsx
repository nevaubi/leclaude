      ...
      <DialogContent size="sm">
        ...
        <div className="space-y-3">
          ...
          {a.type !== "redaction" && a.type !== "signature" && (
            <div><Label className="text-xs">Color</Label><div className="mt-1 flex flex-wrap gap-1.5">{ANNOTATION_COLORS.map((c) => <button key={c.id} onClick={() => setColor(c.hex)} className={cn("size-6 rounded-full border border-black/15 cursor-pointer", color.toLowerCase() === c.hex.toLowerCase() && "ring-2 ring-ring ring-offset-1 ring-offset-background")} style={{ background: c.hex }} aria-label={c.label} />)}</div></div>
          )}
