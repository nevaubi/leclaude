...
import { getDocument, intelDocuments, intelEntities, intelInsights, intelRelations, upsertDocument } from "@/modules/intel/store";
...
import type { IntelDocument, IntelTimelineEntry } from "@/modules/intel/types";
...
describe("pure helpers", () => {
  ...
  it("ranks insights by recency, relevance, confidence and watches", () => {
    ...
    const base = { status: "published" as const, confidence: 0.8, updatedAt: now, createdAt: now, kind: "trend" as const };
    const mine = { ...base, scope: { matterId: "m1", entityIds: [] } };
    const active = { ...base, scope: { matterId: "m2", entityIds: [] } };
    const firm = { ...base, scope: { entityIds: [] } };
    const other = { ...base, scope: { matterId: "m3", entityIds: [] } };
    const old = { ...mine, updatedAt: "2026-07-01T00:00:00.000Z" };
    const dismissed = { ...mine, status: "dismissed" as const };
    const ctx = { userId: "u", matterId: "m1", activeMatterIds: ["m2"], watchedTargets: ["e1"], now };
    const s = (i: typeof mine) => scoreInsight(i, ctx).score;
